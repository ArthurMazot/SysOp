...
PCB::PCB(int i, int *tab, string n){
    nome = n;
    id = i;
    tabPag = tab;
    exec = 1;}
...
char GP::criaProcesso(Program *prog){
    ...
    for(int i = 1; i < tabPag[0] + (offset == 0); i++)
        for(int j = 0; j < tamPag; j++){
            if((i == (tabPag[0] - (offset != 0))) && (j == offset)) break;
            mem->pos[tabPag[i]*tamPag + j] = prog->image[(i-1)*tamPag + j];}

    prontos.push_back(new PCB(prontos.size()+1, tabPag, prog->name));
    return 1;}
